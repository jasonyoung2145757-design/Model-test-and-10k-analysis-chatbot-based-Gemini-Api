...
Rules:
1. If the answer is not supported by the context, output exactly: Not found in filings.
2. Output only the final answer.
3. Use one sentence only.
...
def extract_answer_from_result(result):
    """
    Robust extraction for ChatOllama / LangChain result objects.
    Important: never fallback to str(result), otherwise the UI gets polluted.
    """
...
def run_model_native_ollama(question, docs, model_name):
    """
    Qwen models use native Ollama API for more stable output extraction.
    """
    print(f"[DEBUG] Preparing native Ollama model: {model_name}")
    ...
    response = chat(
        ...
        options={
            "temperature": TEMPERATURE,
            "num_predict": get_model_max_tokens(model_name),
        }
    )

    latency = time.time() - start
    print(f"[DEBUG] Native Ollama completed: {model_name} | Latency: {latency:.2f}s")
    print(f"[DEBUG] Native Ollama response: {repr(response)}")
    ...
    if not answer:
        print(f"[DEBUG] Empty native Ollama answer for {model_name}")
        answer = "No extractable answer returned."
    ...
    print(f"[DEBUG][NATIVE OLLAMA RESPONSE TYPE] {type(response)}")
    print(f"[DEBUG][NATIVE OLLAMA RESPONSE] {repr(response)}")

def run_model_langchain(question, docs, model_name):
    """
    DeepSeek keeps the original ChatOllama + LangChain path.
    """
    print(f"[DEBUG] Preparing LangChain model: {model_name}")
    ...
    print(f"[DEBUG] Invoking LangChain model: {model_name}")
    start = time.time()
    result = llm.invoke(prompt)
    latency = time.time() - start
    print(f"[DEBUG] LangChain model completed: {model_name} | Latency: {latency:.2f}s")

    answer = extract_answer_from_result(result)

    if not answer:
        print(f"[DEBUG] Empty extracted answer for {model_name}")
        answer = "Not found in filings."
...
def run_model(question, docs, model_name):
    print(f"[DEBUG][ROUTER] Routing model: {model_name}")

    if model_name.startswith("qwen"):
        print(f"[DEBUG][ROUTER] Using native ollama for: {model_name}")
        return run_model_native_ollama(question, docs, model_name)

    print(f"[DEBUG][ROUTER] Using langchain for: {model_name}")
    return run_model_langchain(question, docs, model_name)
...
def append_benchmark_rows(question, results):
    ...
    for r in results:
        clean_ans = clean_answer(r.get("answer", "")) or "No extractable answer returned."
        ...
        try:
            ...
            for model in MODELS:
                ...
                try:
                    ...
                    answer_text = clean_answer(r["answer"]) or "No extractable answer returned."
            ...
            for r in results:
                summary_rows.append({
                    ...
                    "Answer": clean_answer(r.get("answer", "")) or "No extractable answer returned.",
                    "Sources": format_sources(r.get("sources", [])),
                    "Error": r.get("error", "")
                })
